...
void fury( player_t* p )
{
  ...
  thane->add_action( "execute,if=buff.sudden_death.up" );
  ...
  thane->add_action( "execute,if=target.health.pct<20" );
  thane->add_action( "wrecking_throw" );
  thane->add_action( "rend,if=dot.rend_dot.remains<6" );
  ...
  thane_aoe->add_action( "rampage,if=buff.enrage.remains<gcd|rage>=80" );
  thane_aoe->add_action( "odyns_fury" );
  thane_aoe->add_action( "bloodbath" );
  thane_aoe->add_action( "execute,if=buff.sudden_death.up" );
  thane_aoe->add_action( "rampage,if=buff.recklessness.up" );
  thane_aoe->add_action( "thunder_clap,if=buff.avatar.up" );
  thane_aoe->add_action( "crushing_blow,if=active_enemies<=3" );
